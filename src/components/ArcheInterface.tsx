...
function LiveReading({ base, previous, label, unit = '' }: { base: number, previous: number, label: string, unit?: string }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', fontFamily: 'monospace', fontSize: '10px', opacity: 0.6, lineHeight: 1, color: '#003D2C' }}>
            <span style={{ textTransform: 'uppercase', letterSpacing: '0.1em', fontSize: '8px', opacity: 0.5, marginBottom: '4px' }}>{label}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span>{formatted} {unit}</span>
                {base > 0 && <span style={{ fontSize: '8px', opacity: 0.4 }}>{isUp ? '↑' : '↓'}</span>}
            </div>
        </div>
    );
}
...
function WaveformReading({ lens, current, previous }: { lens: LensType, current: number, previous: number }) {
  ...
  return (
    <div style={{ position: 'relative', width: '300px', height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <svg width="300" height="80" style={{ overflow: 'visible', position: 'absolute', zIndex: 10 }}>
            ...
            <path ref={activePathRef} fill="none" stroke="#003D2C" strokeWidth="1.5" strokeLinecap="round" style={{ filter: 'drop-shadow(0 0 3px rgba(0,61,44,0.1))' }} />
        </svg>
    </div>
  );
}
...
export function ArcheInterface({ onExit }: ArcheInterfaceProps) {
  ...
  return (
    <div style={{
        ...
        backgroundColor: '#FAF9F6', color: '#003D2C', display: 'flex', flexDirection: 'column',
        alignItems: 'center', justifyContent: 'center', transition: 'background-color 1s ease'
    }}>

      {/* GLOBAL MOOD FILTER */}
      <motion.div
        style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, pointerEvents: 'none', zIndex: 20, mixBlendMode: 'multiply' }}
      ...
      <div style={{
          ...
          transition: 'opacity 1s ease', opacity: isClarte ? 0.02 : 0.06,
      ...
      <nav style={{ position: 'absolute', top: '24px', left: 0, width: '100%', padding: '0 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 50 }}>
        <button onClick={onExit} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.15em', opacity: 0.6, border: 'none', background: 'transparent', cursor: 'pointer', color: '#003D2C' }}>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10, width: '100%', height: '100%', pointerEvents: 'none' }}>
        {/* Background Field */}
        <motion.div
            style={{ position: 'absolute', width: '600px', height: '600px', borderRadius: '50%', border: '1px solid #003D2C', opacity: 0.03 }}
        ...
        <motion.div
            ...
            animate={{
                width: isAlignement ? 320 : 260, height: isAlignement ? 180 : 260,
                scale: isMouvement ? 1.2 : 1, x: isOmbre ? -20 : 0,
                opacity: isClarte ? 0.08 : 0.04, filter: isClarte ? "blur(0px)" : "blur(40px)",
        ...
        <div style={{ position: 'absolute', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <motion.div
                ...
                animate={{
                    width: isMouvement ? 300 : 200, height: isMouvement ? 300 : 200,
                    opacity: isClarte ? 0.4 : 0.1, borderColor: isClarte ? "#003D2C" : "transparent",
                }}
                transition={{ duration: 1.5, ease: "easeInOut" }}
            />
            <motion.div
            ...
            >
                <motion.div
                    style={{ position: 'absolute', top: 0, left: '50%', transform: 'translate(-50%, -2px)', width: '4px', height: '4px', backgroundColor: '#003D2C', borderRadius: '50%' }}
             ...
             {isAlignement && (
                 <motion.div
                    ...
                    style={{ position: 'absolute', width: '400px', height: '1px', backgroundColor: '#003D2C', opacity: 0.1 }}
                 />
             )}
        </div>
        {/* Center Dot */}
      ...
      {/* AXIS READING & SENTENCE - FIXED POSITIONING ABOVE NAV */}
      <div style={{ position: 'absolute', bottom: '140px', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', zIndex: 40, pointerEvents: 'none' }}>
        <AnimatePresence>
            {!isNeutral && (
                <motion.div
                    ...
                    style={{ marginBottom: '16px', position: 'relative', width: '300px', display: 'flex', justifyContent: 'center' }}
                >
        ...
        <AnimatePresence mode="wait">
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
            <motion.p
                ...
                style={{
                    fontFamily: 'serif', fontSize: '18px', letterSpacing: '0.05em',
                    color: '#003D2C', fontStyle: 'italic', fontWeight: 300,
                    lineHeight: 1.6, textAlign: 'center', padding: '0 16px', minHeight: '32px'
                }}
            >
                {visibleSentence}
            </motion.p>
             ...
             {isQuestUnlocked && LENS_CONFIG[activeLens].unlockText && (
                <motion.button
                    ...
                    style={{
                        pointerEvents: 'auto', marginTop: '8px', display: 'flex', alignItems: 'center', gap: '12px',
                        padding: '8px 16px', border: '1px solid rgba(0,61,44,0.2)', backgroundColor: '#FAF9F6',
                        borderRadius: '2px', cursor: 'pointer', boxShadow: '0 1px 2px rgba(0,0,0,0.05)'
                    }}
                >
                    <Sparkles size={12} style={{ color: '#003D2C', opacity: 0.6 }} />
                    <span style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', color: '#003D2C' }}>{LENS_CONFIG[activeLens].unlockText}</span>
                </motion.button>
            )}
          </div>
        </AnimatePresence>
      ...
      {/* LENS SELECTOR - FIXED POSITIONING BOTTOM */}
      <div style={{ position: 'absolute', bottom: '30px', width: '100%', padding: '0 24px' }}>
        <div style={{
            display: 'flex', justifyContent: 'center', gap: '32px', overflowX: 'auto',
            paddingBottom: '16px', alignItems: 'flex-end', height: '80px',
          ...
          {(Object.keys(LENS_CONFIG) as LensType[]).filter(k => k !== 'NEUTRAL').map((lensKey) => {
             ...
             return (
                <button
                    ...
                    style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 'max-content', cursor: 'pointer', background: 'none', border: 'none' }}
                >
                <motion.div
                    style={{ position: 'absolute', bottom: '100%', marginBottom: '12px', width: '1px', background: 'linear-gradient(to top, #003D2C, transparent)', opacity: 0.3 }}
                    initial={{ height: 0 }}
                ...
                <div style={{
                    fontSize: '10px', textTransform: 'uppercase', fontFamily: 'sans-serif',
                    transition: 'all 0.5s ease-out', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px',
                    color: isActive ? '#003D2C' : 'rgba(0,61,44,0.4)',
                    fontWeight: isActive ? 500 : 300,
                    letterSpacing: isActive ? 'normal' : '0.25em',
                    transform: isActive ? 'scale(1.1)' : 'scale(1)'
                }}>
                    {LENS_CONFIG[lensKey].label}
                    {hasActivity && !isActive && (
                        <div style={{ width: '2px', height: '2px', backgroundColor: 'rgba(0,61,44,0.4)', borderRadius: '50%', marginTop: '4px' }} />
                    )}
                </div>
                </button>
             );
