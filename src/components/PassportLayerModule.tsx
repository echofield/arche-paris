...
export function PassportLayerModule({ passport, fund, reliquaire, isSecondaryWeight = false }: PassportLayerModuleProps) {
  ...
  const containerStyle: React.CSSProperties = {
    ...
    border: '1px solid rgba(0, 61, 44, 0.06)',
    ...transitionStyle,
  };
  ...
  const titleStyle: React.CSSProperties = {
    ...
    letterSpacing: '0.1em',
    color: '#003D2C',
    opacity: isSecondaryWeight ? 0.5 : 0.7,
...
function ReliquairePlaceholder({ phase, faint }: { phase: MonumentPhase; faint: boolean }) {
  ...
  const phaseToStroke = (p: MonumentPhase): number => {
    switch (p) {
      case 'reserve': return 1;
      case 'chamber': return 1.5;
      case 'sanctuary': return 2;
    ...
    >
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block' }}>
        <rect
          ...
          fill="none"
          stroke="#003D2C"
          strokeWidth={phaseToStroke(phase)}
        />
        {phase === 'chamber' && (
          <line x1={size / 2} y1={stroke} x2={size / 2} y2={size - stroke} stroke="#003D2C" strokeWidth={0.5} opacity={0.6} />
        )}
        {phase === 'sanctuary' && (
          <>
            <line x1={size / 2} y1={stroke} x2={size / 2} y2={size - stroke} stroke="#003D2C" strokeWidth={0.5} opacity={0.6} />
            <line x1={stroke} y1={size / 2} x2={size - stroke} y2={size / 2} stroke="#003D2C" strokeWidth={0.5} opacity={0.6} />
          </>
        )}
        {phase === 'archive' && (
          <>
            <line x1={size / 2} y1={stroke} x2={size / 2} y2={size - stroke} stroke="#003D2C" strokeWidth={0.5} opacity={0.7} />
            <line x1={stroke} y1={size / 2} x2={size - stroke} y2={size / 2} stroke="#003D2C" strokeWidth={0.5} opacity={0.7} />
            <circle cx={size / 2} cy={size / 2} r={4} fill="none" stroke="#003D2C" strokeWidth={0.5} opacity={0.5} />
          </>
        )}
      </svg>
    </div>
