...
// ----- /pair -----
// Allowed only if activated_at IS NOT NULL AND device_secret_hash IS NULL. One-time only.
...
// Plan: journal max 100 per card/hour, 500 per IP/hour; trace same or lower
...
// ----- /journal/note (GET: load My Paris note) -----
...
// ----- /journal/note (POST: save My Paris note) -----
app.post("/journal/note", async (c) => {
  ...
  if (existing?.id) {
    ...
    if (iErr) {
      if (iErr.code === "23505") return c.json({ ok: true }); // dedupe
...
// ----- /journal/entries (POST: append one entry) -----
app.post("/journal/entries", async (c) => {
  ...
  if (error) {
    if (error.code === "23505") return c.json({ ok: true }); // dedupe
    return c.json({ error: "Failed to append entry" }, 500);
  }
  return c.json({ ok: true });
});

// ----- /journal/insert (plan alias: single insert with content, place_id) -----
app.post("/journal/insert", async (c) => {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) {
    if (error.code === "23505") return c.json({ success: true, entry: null }); // dedupe
...
app.post("/trace/leave", async (c) => {
  ...
  if (error) {
    if (error.code === "23505") return c.json({ success: true, message: "Trace laissée." }); // dedupe
...
// Append-only; client can only create status='pending'. Server never accepts status='verified' from client.
