...
serve(async (req) => {
  ...
  try {
    ...
    if (!run) {
      await logRejection(auth.userId, 'rituals-abort', body, runError!.code, { run_id });
    ...
    const abortResult = validateRitualAbort(accuracy_m, duration_ms);
    if (!abortResult.valid) {
      await logRejection(auth.userId, 'rituals-abort', body, abortResult.error!.code, {
        ...abortResult.error!.details,
        run_id,
        duration_ms
      });
      return new Response(JSON.stringify({
        ok: false,
        code: abortResult.error!.code,
        message: abortResult.error!.message,
        details: abortResult.error!.details
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
