...
serve(async (req) => {
  ...
  try {
    ...
    const { attempt_id, challenge_id, score_inputs, client_ts, idempotency_key } = body;
    ...
    const result = await writeEvent({
      userId: auth.userId,
      eventType: 'challenge_attempt_completed',
      payload: { attempt_id, challenge_id, score_inputs: score_inputs || {}, client_ts },
