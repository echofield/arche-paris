-- ARCHÉ Decision Events
-- Adds decision_made event type for Decision Nodes
-- Silently applies Aura deltas based on payload

-- Update the projector function to handle decision_made events
...
begin
  ...
  -- ---- Decision made (silent Aura modification) ----
  if ev.event_type = 'decision_made' then
    v_node_id := ev.payload ->> 'node_id';
    v_choice := ev.payload ->> 'choice';

    -- Deltas come from payload, not fixed formulas
    d_presence := coalesce((ev.payload ->> 'd_presence')::int, 0);
    d_wisdom := coalesce((ev.payload ->> 'd_wisdom')::int, 0);
    d_shadow := coalesce((ev.payload ->> 'd_shadow')::int, 0);

    -- Log delta (audit trail)
    insert into public.complexion_deltas(user_id, event_id, d_presence, d_wisdom, d_shadow, reason)
    values (ev.user_id, ev.event_id, d_presence, d_wisdom, d_shadow, 'decision:' || coalesce(v_node_id, 'unknown'))
    on conflict do nothing;
    ...
    -- Silent application - no notification
